...
package RewardedIvan.asteroidclient.renderer.text;
...
public class VanillaTextRenderer implements TextRenderer {
    ...
    public double getWidth(String text, int length, boolean shadow) {
        ...
        return (AsteroidClient.mc.textRenderer.getWidth(string) + (shadow ? 1 : 0)) * scale;
    }

    @Override
    public double getHeight(boolean shadow) {
        return (AsteroidClient.mc.textRenderer.fontHeight + (shadow ? 1 : 0)) * scale;
    }
    ...
    public double render(String text, double x, double y, Color color, boolean shadow) {
        ...
        double width = AsteroidClient.mc.textRenderer.draw(text, (float) (x / scale), (float) (y / scale), color.getPacked(), shadow, emptyMatrix, immediate, true, 0, 15728880);

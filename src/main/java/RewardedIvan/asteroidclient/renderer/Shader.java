...
public class Shader {
    ...
    public Shader(String vertPath, String fragPath) {
        ...
        if (vertError != null) {
            AsteroidClient.LOG.error("Failed to compile vertex shader (" + vertPath + "): " + vertError);
        ...
        if (fragError != null) {
            AsteroidClient.LOG.error("Failed to compile fragment shader (" + fragPath + "): " + fragError);
        ...
        if (programError != null) {
            AsteroidClient.LOG.error("Failed to link program: " + programError);
    ...
    private String read(String path) {
        try {
            return IOUtils.toString(AsteroidClient.mc.getResourceManager().getResource(new Identifier(AsteroidClient.MOD_ID, "shaders/" + path)).getInputStream(), StandardCharsets.UTF_8);
        } catch (IOException e) {
            e.printStackTrace();
            return "";
        }

...
public class AccountCache implements ISerializable<AccountCache> {
    ...
    public boolean loadHead(String url) {
        try {
            ...
            AsteroidClient.LOG.error("Failed to read skin url (" + url + ").");
            return false;
        }
    }
    ...
    public static void loadSteveHead() {
        try {
            ByteBuffer data = TextureUtil.readResource(AsteroidClient.mc.getResourceManager().getResource(new Identifier(AsteroidClient.MOD_ID, "textures/steve.png")).getInputStream());
            data.rewind();

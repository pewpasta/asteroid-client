...
public class PlayerArgumentType implements ArgumentType<PlayerEntity> {
    ...
    static {
        if (AsteroidClient.mc.world != null) {
            EXAMPLES = AsteroidClient.mc.world.getPlayers()
                .stream()
                .limit(3)
    ...
    public PlayerEntity parse(StringReader reader) throws CommandSyntaxException {
        ...
        for (PlayerEntity p : AsteroidClient.mc.world.getPlayers()) {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        return CommandSource.suggestMatching(AsteroidClient.mc.world.getPlayers().stream().map(PlayerEntity::getEntityName), builder);
    }

...
public class PlayerListEntryArgumentType implements ArgumentType<PlayerListEntry> {
    ...
    static {
        if (AsteroidClient.mc.getNetworkHandler() != null) {
            EXAMPLES = AsteroidClient.mc.getNetworkHandler().getPlayerList()
                .stream()
                .limit(3)
    ...
    public PlayerListEntry parse(StringReader reader) throws CommandSyntaxException {
        ...
        for (PlayerListEntry p : AsteroidClient.mc.getNetworkHandler().getPlayerList()) {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        return CommandSource.suggestMatching(AsteroidClient.mc.getNetworkHandler().getPlayerList().stream().map(playerListEntry -> playerListEntry.getProfile().getName()), builder);
    }

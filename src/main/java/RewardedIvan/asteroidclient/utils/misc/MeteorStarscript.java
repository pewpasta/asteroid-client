...
public class MeteorStarscript {
    ...
    public static void init() {
        ...
        // General
        ss.set("version", Value.string(AsteroidClient.VERSION != null ? (AsteroidClient.DEV_BUILD.isEmpty() ? AsteroidClient.VERSION.toString() : AsteroidClient.VERSION + " " + AsteroidClient.DEV_BUILD) : ""));
        ...
        // Meteor
        ss.set("asteroid", Value.map(new ValueMap()
    ...
    private static Value isModuleActive(Starscript ss, int argCount) {
        if (argCount != 1) ss.error("asteroid.is_module_active() requires 1 argument, got %d.", argCount);

        Module module = Modules.get().get(ss.popString("First argument to asteroid.is_module_active() needs to be a string."));

...
public class EntityTooltipComponent implements MeteorTooltipData, TooltipComponent {
    ...
    protected void renderEntity(MatrixStack matrices, int x, int y) {
        if (AsteroidClient.mc.player == null) return;
        float size = 24;
        ...
        setupAngles();
        EntityRenderDispatcher entityRenderDispatcher = AsteroidClient.mc.getEntityRenderDispatcher();
        quaternion2.conjugate();
        entityRenderDispatcher.setRotation(quaternion2);
        entityRenderDispatcher.setRenderShadows(false);
        VertexConsumerProvider.Immediate immediate = AsteroidClient.mc.getBufferBuilders().getEntityVertexConsumers();
        entity.age = AsteroidClient.mc.player.age;

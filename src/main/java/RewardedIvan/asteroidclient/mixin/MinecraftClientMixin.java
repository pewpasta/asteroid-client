...
import RewardedIvan.asteroidclient.utils.misc.MeteorStarscript;
import RewardedIvan.asteroidclient.AsteroidClient;
import RewardedIvan.asteroidclient.events.entity.player.ItemUseCrosshairTargetEvent;
...
import RewardedIvan.asteroidclient.events.game.ResourcePacksReloadedEvent;
import RewardedIvan.asteroidclient.events.game.WindowResizedEvent;
import RewardedIvan.asteroidclient.events.world.TickEvent;
import RewardedIvan.asteroidclient.gui.WidgetScreen;
import RewardedIvan.asteroidclient.mixininterface.IMinecraftClient;
import RewardedIvan.asteroidclient.systems.config.Config;
import RewardedIvan.asteroidclient.systems.modules.Modules;
import RewardedIvan.asteroidclient.systems.modules.render.UnfocusedCPU;
...
public abstract class MinecraftClientMixin implements IMinecraftClient {
    ...
    private void onInit(CallbackInfo info) {
        AsteroidClient.INSTANCE.onInitializeClient();
        firstFrame = true;
    }
    ...
    private void onPreTick(CallbackInfo info) {
        ...
        getProfiler().push("asteroid-client_pre_update");
        AsteroidClient.EVENT_BUS.post(TickEvent.Pre.get());
        getProfiler().pop();
    ...
    private void onTick(CallbackInfo info) {
        getProfiler().push("asteroid-client_post_update");
        AsteroidClient.EVENT_BUS.post(TickEvent.Post.get());
        getProfiler().pop();
    }
    ...
    private HitResult doItemUseMinecraftClientCrosshairTargetProxy(MinecraftClient client) {
        return AsteroidClient.EVENT_BUS.post(ItemUseCrosshairTargetEvent.get(client.crosshairTarget)).target;
    }
    ...
    private CompletableFuture<Void> onReloadResourcesNewCompletableFuture(CompletableFuture<Void> completableFuture) {
        completableFuture.thenRun(() -> AsteroidClient.EVENT_BUS.post(ResourcePacksReloadedEvent.get()));
    ...
    private void onResolutionChanged(CallbackInfo info) {
        AsteroidClient.EVENT_BUS.post(WindowResizedEvent.get());
    }
